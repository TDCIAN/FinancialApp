...
class FinancialAppTests: XCTestCase {
    ...
    func testResult_givenWinningAssetAndDCAIsUsed_expectPositiveGains() {
        ...
        XCTAssertEqual(result.currentValue, 16614.98, accuracy: 0.1)
        XCTAssertEqual(result.gain, 4114.98, accuracy: 0.1)
        XCTAssertEqual(result.yield, 0.3292, accuracy: 0.0001)
    }
    
    func testResult_givenWinningAssetAndDCAIsNotUsed_expectPositiveGains() {
        // given
        let initialInvestmentAmount: Double = 5000
        let monthlyDollarCostAveargingAmount: Double = 0
        let initialDateOfInvestmentIndex: Int = 5
        let asset = buildWinningAsset()
        // when
        let result = sut.calculate(asset: asset,
                                   initialInvestmentAmount: initialInvestmentAmount,
                                   monthlyDollorCostAverageAmount: monthlyDollarCostAveargingAmount,
                                   initialDateOfInvestmentIndex: initialDateOfInvestmentIndex)
    ...
    func testResult_givenLosingAssetAndDCAIsUsed_expectNegativeGains() {
        // given
        let initialInvestmentAmount: Double = 5000
        let monthlyDollarCostAveargingAmount: Double = 1500
        let initialDateOfInvestmentIndex: Int = 5
    ...
    private func buildWinningAsset() -> Asset {
        ...
        let meta = buildMeta()
        let timeSeries: [String: OHLC] = ["2021-01-25":OHLC(open: "100", close: "100", adjustedClose: "110"),
